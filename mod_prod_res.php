...
$idsImgDelete=$_POST['idsImgDelete'];
...
{  
		...
		$db->beginTransaction();
		$c="UPDATE 'producto' SET 'nombre'='".$nombre."','precio'='".$precio."','cantidad'='".$cantidad."','descuento'='".$descuento."' WHERE 'id'=".$idProducto;
		$consulta= "update producto set nombre='".$nombre."',precio='".$precio."',cantidad='".$cantidad."',descuento='".$descuento."' where id=".$idProducto;
		...
		elseif ($_FILES['archivo']['error'][0]==4 && $_FILES['archivo']['error'][1]==4 && $_FILES['archivo']['error'][2]==4) {
			//no se va  subir ningun archivo
		}

        $consulta="update caracteristicas set caracteristica='".$caracteristicas."',descripcion='".$descripcion."' where id_producto=".$idProducto;
        $res=$db->exec($consulta);
		$consulta="update tag_producto set id_tag='".$categoria."' where id_producto=".$idProducto;
		$res=$db->exec($consulta);

        $token = strtok($idsImgDelete, ",");
		while ($token !== false)
		{
			$consulta="select src from imagen where id=".$token;
			...
			$consulta="delete from imagen where id=".$token;
			$res=$db->exec($consulta);
			$consulta="delete from imagen_producto where id_imagen=".$token;
			$res=$db->exec($consulta);
			$token = strtok(",");
		}
